...
#define FUNCION_BEGIN if(debug)fprintf(debugLog, "{%s begins...}\n", __FUNCTION__);
#define FUNCION_END if(debug)fprintf(debugLog, "{%s ends...}\n", __FUNCTION__);
...
/*
 * Tuning frequency in units of 62.5 kHz, or if the struct v4l2_tuner or struct v4l2_modulator capability flag
 * V4L2_TUNER_CAP_LOW is set, in units of 62.5 Hz. A 1 Hz unit is used when the capability flag
...
static void radio_mute(int fd, int mute, int alsa_loopback) {
	FUNCION_BEGIN
	if (debug) {
		fprintf(debugLog, "{%s}::mute=%d;alsa_loopback=%d;\n", __FUNCTION__,
				mute, alsa_loopback);
	}
	...
	if (alsa_loopback) {
		if (mute)
			...
			alsa_thread_startup(alsa_playback, alsa_capture, alsa_latency,
					debugLog, debug);
		}
	}
#endif
...
static int radio_getsignal_n_stereo(int fd) {
	FUNCION_BEGIN
	struct v4l2_tuner tuner;
	int i, asterisks;
...
static void print_freq(int band, int ifreq, int muted) {
	...
	if (alsa_capture) {
		mvwprintw(wfreq, 5, 2, "device %s", alsa_capture);
	} else {
		mvwprintw(wfreq, 5, 2, "freqFact %d", freqfact);
	}
	wrefresh(wfreq);
}
...
static int check_band_n_freq(int *band, int ifreq, const char *extra_msg) {
	...
	if (*band == -1) {
		*band = 0;
		...
		fprintf(debugLog, "Invalid band '%d'%s\n", *band, extra_msg);
		return -1;
	}
	if (v4l2_f < bands[*band].rangelow || v4l2_f > bands[*band].rangehigh) {
		fprintf(debugLog, "Invalid freq '%d'%s\n", ifreq, extra_msg);
		return -1;
	}
	return 0;
}
...
static void foundone(int m) {
	...
	if (i == stations || g[m] > g[astation[i]]) {
		if (i == stations)
			stations = i + 1;
		astation[i] = m;
		fprintf(debugLog, "Station %2d: %6.2f MHz - %.2f\n", i, freq, g[m]);
	}
}

static void maxi(int m) {
	FUNCION_BEGIN
	int i, l, r;
	float freq, halbwert;
	...
	if (debug) {
		freq = FREQ_MIN_MHZ + i * FREQ_STEP_MHZ;
		fprintf(debugLog, "Right  i %d %f %f\n", i, freq, g[i]);
	}
	r = i;
	m = (l + r) / 2;
	if (debug) {
		freq = FREQ_MIN_MHZ + m * FREQ_STEP_MHZ;
		fprintf(debugLog, "Middle m %d %f %f\n", m, freq, g[m]);
	}
	foundone(m);
}
...
static float get_baseline(float ming, float maxg) {
	FUNCION_BEGIN
	int unt, i;
	float nullinie = 0, u;

	if (debug)
		fprintf(debugLog, "get_baseline:  min=%f max=%f\n", ming, maxg);
	for (u = ming; u < maxg; u += 0.1) {
		...
		if (unt > (g_len * 7 / 8)) {
			fprintf(debugLog, "baseline at %.2f\n", u);
			nullinie = u;
			break;
		}
		if (debug)
			fprintf(debugLog, "%f %d\n", u, unt);
	}
	return nullinie;
}

static void findstations(void) {
	FUNCION_BEGIN
...
static void do_scan(int fd, int scan, int write_config) {
	...
	for (i = 0; i < g_len; i++) {
		...
		if (scan > 1)
			fprintf(fmap, "%f %d\n", ifreq / 1e6, s);
		fprintf(debugLog, "scanning: %6.2f MHz - %6d\r", ifreq / 1e6, s);
	}
	fprintf(debugLog, "%40s\r", "");
	if (scan > 1)
		fclose(fmap);
	findstations();
...
static void usage(FILE *out) {
	FUNCION_BEGIN
	fprintf(out, "radio -- interactive ncurses radio application\n"
			"usage:\n"
			"  radio [ options ]\n"
			"\n"
			"options:\n"
...
static void draw_wbands(void) {
	...
	for (i = 0; i < no_bands; i++) {
		if (bands[i].rangehigh < (2 * freqfact))
			mvwprintw(wbands, i + 1, 2, "%d.%10.10s %6.1f - %6.1f kHz%s", i + 1,
					modulation_label(i), bands[i].rangelow * (1e3 / freqfact),
					bands[i].rangehigh * (1e3 / freqfact),
					(i == band) ? " (*)" : "");
		else
			mvwprintw(wbands, i + 1, 2, "%d.%10.10s %6.2f - %6.2f MHz%s", i + 1,
					modulation_label(i), (float) bands[i].rangelow / freqfact,
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		...
		case 'd':
			debug = 1;
			if (debugLog == stderr) {
				debugLog = fopen("/tmp/debug.log", "w");
				if (debugLog == NULL) {
					debugLog = stderr;
				} else {
					fprintf(debugLog, "Debug begins!\n");
				}
			}
			break;
		case 'S':
		...
		case 'b':
			...
			if (band >= 1) {
				...
				fprintf(debugLog, "Invalid band: '%s'\n", optarg);
				band = -3;
			}
			break;
		case 'f':
	...
	if (-1 == (fd = open(device, O_RDONLY))) {
		fprintf(debugLog, "open %s: %s\n", device, strerror(errno));
		exit(1);
	}

#if defined(HAVE_ALSA)
	if (debug)fprintf(debugLog, "alsa_loopback=%d; alsa_capture=%s\n", alsa_loopback, alsa_capture?alsa_capture:"NULL");
	if (alsa_loopback && alsa_capture == NULL) {
		...
		{
			alsa_loopback = 0;
			if (debug)fprintf(debugLog, "alsa_loopback is set to 0\n");
		}
	...
	if (band >= no_bands) {
		fprintf(debugLog, "Invalid band: '%d'\n", band);
		band = -3;
	}
	if (band <= -2) {
		fprintf(debugLog, "Possible band values:\n");
		for (i = 0; i < no_bands; i++) {
			if (bands[i].rangehigh < (2 * freqfact))
				fprintf(debugLog, "%d.%10.10s %6.1f - %6.1f kHz\n", i + 1,
						modulation_label(i),
						bands[i].rangelow * (1e3 / freqfact),
						bands[i].rangehigh * (1e3 / freqfact));
			else
				fprintf(debugLog, "%d.%10.10s %6.2f - %6.2f MHz\n", i + 1,
						modulation_label(i),
	...
	if (newfreq) {
		if (band >= 0 && bands[band].rangehigh < (2 * freqfact)) {
			fprintf(debugLog, "Tuning to %.2f kHz\n", newfreq);
			ifreq = newfreq * 1e3;
		} else {
			fprintf(debugLog, "Tuning to %.2f MHz\n", newfreq);
			ifreq = newfreq * 1e6;
		}
...
#if defined(HAVE_ALSA)
	if (alsa_loopback)
		fprintf(debugLog, "Using alsa loopback: cap: %s (%s), out: %s\n",
